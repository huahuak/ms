  ...
  <div class="add">
    <el-card class="add-container">
      <el-form :model="goodForm" :rules="rules" ref="goodRef" label-width="200px" class="goodForm" size="large">
        ...
        <el-form-item required label="期限(月)">
       ...
       <el-form-item label="秒杀时间" >
        ...
        <el-form-item>
         <el-button type="primary" @click="submitNewActivity">立即修改</el-button>
        </el-form-item>
      </el-form>
    </el-card>
  </div>
...
import { changeDepositAct } from '@/api/table'
export default {
  ...
  setup() {
	...
	const submitNewActivity = () => {
	  ...
	  state.goodForm.activity_endTime = state.formTime[1]
		...
		changeDepositAct(state.goodForm,state.id).then((res)=>{
			if(res.code==200) {
				ElMessage({
					message:res.message,
					type:'success',
	...
	onBeforeMount(()=>{
		state.goodForm =  JSON.parse(route.query.data)
    state.id = state.goodForm.activity_id
		state.formTime[0] =  state.goodForm.activity_startTime
		state.formTime[1] =  state.goodForm.activity_endTime
	})
    return {
      ...toRefs(state),
	  changeQuantity,
